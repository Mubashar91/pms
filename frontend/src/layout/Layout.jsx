...
export default function Layout({ currentPageName }) {
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      // Since we're using static data, we'll just update the state
      const newTeam = {
  ...
  const handleDeleteTeam = async (teamId) => {
    ...
    try {
      ...
      setProjects(projects.filter(p => p.team_id !== teamId));
    } catch (error) {
                          ...
                          <SidebarMenu className="space-y-1 py-1">
                            {projects.filter(p => p.team_id === team.id).map(p => (
                               <SidebarMenuItem key={p.id}>
                                ...
                                >
                                  <Link to={createPageUrl(`ProjectDetails?id=${p.id}`)} className="flex items-center gap-3 text-sm truncate">
