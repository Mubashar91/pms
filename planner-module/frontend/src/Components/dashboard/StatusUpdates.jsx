...
import  StatusUpdate  from "@/entities/StatusUpdate.json";
...
const loadUpdates = useCallback(async () => {
  try {
    const allUpdates = await StatusUpdate.list("-created_date", 50);
    const filteredUpdates = selectedProjectId 
      ? allUpdates.filter(update => update.project_id === selectedProjectId)
      : allUpdates;
 ...
 useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await StatusUpdate.create(newUpdate);
      setNewUpdate({
        project_id: selectedProjectId || '',
        update_text: '',
        update_type: 'General',
